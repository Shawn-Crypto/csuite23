-- Supabase Database Schema for Production (Security Fixed)
-- Complete Indian Investor Course Platform
-- Created: 2025-01-11
-- Fixed: Security definer issues and search path warnings
...
-- Create updated_at trigger function with explicit search_path (SECURITY FIX)
...
DROP POLICY IF EXISTS "Allow service role all access on webhook_events" ON webhook_events;
CREATE POLICY "Allow service role all access on webhook_events" ON webhook_events
    FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Allow service role all access on webhook_errors" ON webhook_errors;
CREATE POLICY "Allow service role all access on webhook_errors" ON webhook_errors
    FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Allow service role all access on lead_captures" ON lead_captures;
CREATE POLICY "Allow service role all access on lead_captures" ON lead_captures
    FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Allow service role all access on purchase_analytics" ON purchase_analytics;
CREATE POLICY "Allow service role all access on purchase_analytics" ON purchase_analytics
    FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Allow service role all access on meta_pixel_events" ON meta_pixel_events;
CREATE POLICY "Allow service role all access on meta_pixel_events" ON meta_pixel_events
    FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Allow service role all access on system_health" ON system_health;
CREATE POLICY "Allow service role all access on system_health" ON system_health
    FOR ALL USING (true) WITH CHECK (true);

-- Create useful views for analytics (SECURITY FIXED - No SECURITY DEFINER)

-- Revenue summary view (invoker rights)
...
-- Conversion funnel view (invoker rights)
DROP VIEW IF EXISTS conversion_funnel;
CREATE VIEW conversion_funnel 
SECURITY INVOKER AS
SELECT 
...
-- Product performance view (invoker rights)
...
BEGIN
    ...
    RAISE NOTICE '🛡️  Security issues resolved: SECURITY INVOKER views, explicit search_path';
END $$;